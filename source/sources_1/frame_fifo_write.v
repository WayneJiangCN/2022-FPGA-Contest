...
begin
	...
	begin
		...
		case(state)
			//idle state,waiting for write write_req_d2 == '1' goto the 'S_ACK'   空闲状态，等待写入write_req_d2 == '1'去'S_ACK'
			S_IDLE:
			begin
			...
			//'S_ACK' state completes the write request response, the FIFO reset, the address latch, and the data length latch//'S_ACK'状态完成写请求响应、FIFO重置、地址锁存和数据长度锁存
			S_ACK:
			begin
				...
				begin
					...
					//FIFO reset
					fifo_aclr <= 1'b1;
					//select valid base address from write_addr_0 write_addr_1 write_addr_2 write_addr_3从“write_addr_0 write_addr_1 write_addr_2 write_addr_3”中选择有效的基址
			...
			begin
				...
				//if the FIFO space is a burst write request, goto burst write state//如果FIFO空间是突发写请求，则转到突发写状态
