...
parameter BUSRT_BITS             = 11;             //external memory user interface burst width
...
wire                            write_req;
wire                            write_req_ack;
//wire                            video_clk;         //video pixel clock视频像素时钟
wire                            video_clk5x;
wire                            hs;
wire                            vs;
wire                            de;
wire[23:0]                      vout_data;
// wire[23:0]                      in_hdmi_24bit_data;
wire                            in_hdmi_24bit_wr;
...
assign write_en = in_hdmi_de;
assign write_data = {in_hdmi_r[7:0] ,in_hdmi_g[7:0] ,in_hdmi_b[7:0],8'd0 };
wire                             sys_clk_g;
...
//);
 ...
 GTP_CLKBUFG video_clk_outbufg
 (
   .CLKOUT                    (video_clk_out                 ),
   .CLKIN                     (video_clk_out_w             )
 );
 GTP_CLKBUFG video_clk5x_outbufg
 (
   .CLKOUT                    (video_clk5x_out                 ),
   .CLKIN                     (video_clk5x_out_w             )
 );
wire rst_n_eep;
...
(
	.pixelclk      (video_clk_out          ),// system clock
	.pixelclk5x    (video_clk5x_out        ),// system clock x5
	.rstin         (~rst_n             ),// reset
...
//图像灰度化，RGB转YCbCr，并提取Y分量
rgb_to_ycbcr ycbcr_ins(
	 .clk              (video_clk_out        ),
	 .i_r_8b           (vout_data[23:16]     ),
	 .i_g_8b           (vout_data[15:8]     ), 
	 .i_b_8b           (vout_data[7:0]      ), 
     .rst_n            (rst_n                          ),
     .i_h_sync         (hs                             ),
     .i_v_sync         (vs                             ),
     .i_data_en        (de                             ),
...
fenqu  u_fenqu(
		...
		.i_data_en(yCbCr_de),
        . o_data(fenqu_data),
        .o_data_en(fenqu_data_en)                      
   );

//MBI5353_top u_MBI5353_top(
   //.sys_clk (sys_clk_g) ,      //系统输入时钟
  // .sys_rst_n (rst_n),      //复位信号，低电平有效  
  // .data_input (fenqu_data),     //ddr数据输入
   //.i_data_en(fenqu_data_en),  
  // .GCLK (GCLK),
  // .DCLK (DCLK),
  // .LE (LE),
  // .spi_mosi (spi_mosi),
...
//系统接口
        ...
        .sys_rst_n        (rst_n),      //复位信号，低电平有效 
 .p_clk            (video_clk_out), 
        .data_input         (fenqu_data),
//用户接口 
        .o_dataen        (fenqu_data_en),
        .GCLK            (GCLK),
        .DCLK            (DCLK),
        .LE              (LE),
       .spi_mosi        (spi_mosi),
...
//CMOS sensor writes the request and generates the read and write address index   CMOS传感器写入请求并生成读写地址索引
cmos_write_req_gen cmos_write_req_gen_m0(
	.rst                        (~rst_n                   ),
	.pclk                       (video_clk              ),
	.cmos_vsync                 (in_hdmi_vs               ),
	.write_req                  (write_req                ),
...
// 	.pdata_i                    (cmos_db                  ),
// 	.de_i                       (in_hdmi_hs                ),
// 	.pdata_o                    (in_hdmi_24bit_data          ),
// 	.hblank                     (                         ),
// 	.de_o                       (in_hdmi_24bit_wr            )
// );

		
//The video output timing generator and generate a frame read data request该视频输出定时发生器并生成一帧读数据请求
...
(
	...
	.read_addr_index            (read_addr_index          ),
	.read_len                   (25'd470000               ),//frame size //frame size 1280*720 * 32 / 64
	.read_en                    (read_en                  ),
	.read_data                  (read_data                ),
	...
	.write_addr_index           (write_addr_index         ),
	.write_len                  (25'd470000              ), //frame size  
	.write_en                   (write_en                 ),
